...
import { getServicesFour, getGlobalContent } from "../../lib/api";
import { customColors } from "../../customdata";

const background = customColors();

export default function AnalyseInnsikt({ pageContent, globalContent }) {
  const {
    pageTitle,
...
export async function getStaticProps() {
  const pageContent = await getServicesFour(); // fetches query
  const globalContent = await getGlobalContent();

  return {
    props: {
      pageContent: pageContent.serviceSubPageFour.data.attributes, // creates a const from toplevel query and serves it as prop
